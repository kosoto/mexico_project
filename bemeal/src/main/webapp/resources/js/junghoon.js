...
junghoon.service = {
			search : x =>{
					$.magnificPopup.open({
								...
								fixedContentPos:false,
								items:{src:
									$('<div/>').addClass('container search-table').attr({id:'search_the_SachalGod'}).append(
										$('<div/>').addClass('search-box rounded').append(
												$('<div/>').addClass('row').append(
													$('<div/>').addClass('col-md-3').append(
														$('<img/>').attr({src:'/web/resources/img/junghoon/j_search.jpg',style:'height:100%; width:100%; margin:5px auto;'})
													),
													$('<div/>').addClass('col-md-3').append(
															$('<img/>').attr({src:'/web/resources/img/junghoon/j_tag.jpg',id:'search2tag',style:'height:100%; width:100%; margin:5px auto;'})
													)
												),
												$('<div/>').addClass('search-keyword'),
												$('<br/>'),$('<br/>'),
												$('<form/>').addClass('form-inline md-form mr-auto mb-4').append(
													$('<input/>').addClass('j_form-control mr-sm-2').attr({type:'text',placeholder:'Search','aria-label':'Search'}),
													$('<button/>').addClass('btn btn-outline-warning btn-rounded btn-sm my-0').attr({type:'submit'}).text('Search')
												),
												$('<div/>').append(
													$('<div/>').append(
														$('<img/>').attr({src:'/web/resources/img/junghoon/실검.png',id:'popular_searches',style:'height:30%; width:30%;'}),
														$('<img/>').attr({src:'/web/resources/img/junghoon/내연인검.png',id:'myage_popular',style:'height:30%; width:30%; margin:5px auto;'})
													),
													$('<ul/>').attr({id:'popular_search_time'}).append(
														$('<li/>').text('1위 GS도시락'),
														$('<li/>').text('2위 치킨도시락'),
														$('<li/>').text('3위 한솥도시락'),
														$('<li/>').text('4위 라면tep'),
								...
								+'<table class="table" id="myTable">'             
									+'<tr>'
										...
										+'<div class="j_btn-group" data-toggle="buttons" >'
											...
											+'</div>&nbsp;'
											+'<div class="btn btn-danger btn-rounded j_btn">'
												+'<input id="j_tag" class="j_scbox" type="checkbox" type="" name="9" autocomplete="off"> 새우'
											+'</div>&nbsp;'
											+'<div class="btn btn-danger btn-rounded j_btn">'
												+'<input id="j_tag" class="j_scbox" type="checkbox" type="" name="10" autocomplete="off"> 고등어'
											+'</div>&nbsp;'
											+'<div class="btn btn-danger btn-rounded j_btn">'
