function k_HomeUI(){
	return 	'<div class="grid-layout">'
	+'<div id="side_grid" class="grid_menu">' //맨처음에나뉨
	+'<div class="k_aside">　' //왼쪽사이드 
	+'<div class="k_navi">　'//여기서부터가 네비들어갈자리
	+'<div class="list-group">'
	+'<div id="k_home_btn">HOME</div>'
	/*+'<button id="cart_btn" type="button" class="list-group-item list-group-item-action list-group-item-warning">장바구니</button>'
	+'<button id="payHis_btn" type="button" class="list-group-item list-group-item-action list-group-item-warning">구매함</button>'
	+'<button id="gift_btn" type="button" class="list-group-item list-group-item-action list-group-item-warning">선물함</button>'
	+'<button id="analysis_btn" type="button" class="list-group-item list-group-item-action list-group-item-warning">취향분석</button>'
	+'<button id="collect_btn" type="button" class="list-group-item list-group-item-action list-group-item-warning">콜렉션</button>'*/
	+' <div id="cart_btn" class="button_base b01_simple_rollover">장바구니</div>'
	+'<div  id="payHis_btn" class="button_base b01_simple_rollover">구매함</div>'
	+'<div  id="gift_btn" class="button_base b01_simple_rollover">선물함</div>'
	+'<div  id="analysis_btn" class="button_base b01_simple_rollover">취향분석</div>'
	+'<div id="collect_btn" class="button_base b01_simple_rollover">콜렉션</div>'
	+'</div>' //list끝
	+'</div>' //여기까지네비
	+'</div>'
	+'</div>'
	+'<div class="grid_main">'
	+'<div id="content_grid">'
	+'<h2>content지롱</h2><br>말<br>말<br>말<br>말<br>말<br>말'
	+'</div>'
	+'</div>'
	+'<div id="side_grid" class="grid_rightside">'
	+'</div>';}
/*<div class="grid-layout">
<div id="side_grid" class="grid_menu">

</div>
<div class="grid_main">
<div id="content_grid">
<h2>content지롱</h2><br>말<br>말<br>말<br>말<br>말<br>말
</div>
</div>
<div id="side_grid" class="grid_rightside"/>*/
